...
public class PhysicsManager {
    ...
    private void initialize() throws Exception {
        ...
        try {
            odeHelperClass = odeClassLoader.loadClass("org.ode4j.ode.OdeHelper");
            dWorldClass = odeClassLoader.loadClass("org.ode4j.ode.DWorld");
            dSpaceClass = odeClassLoader.loadClass("org.ode4j.ode.DSpace");
            dBodyClass = odeClassLoader.loadClass("org.ode4j.ode.DBody");
            dMassClass = odeClassLoader.loadClass("org.ode4j.ode.DMass");
            dGeomClass = odeClassLoader.loadClass("org.ode4j.ode.DGeom");
            dJointGroupClass = odeClassLoader.loadClass("org.ode4j.ode.DJointGroup");
            
            // Box와 Plane 클래스
            try {
                dBoxClass = odeClassLoader.loadClass("org.ode4j.ode.DBox");
            } catch (ClassNotFoundException e) {
                dBoxClass = dGeomClass; // fallback
            }
            try {
                dPlaneClass = odeClassLoader.loadClass("org.ode4j.ode.DPlane");
            } catch (ClassNotFoundException e) {
    ...
    public void step(float dt) {
        ...
        try {
            // 충돌 감지 (space collide)
            // 간단한 구현 - 실제로는 nearCallback이 필요
            if (space != null) {
                try {
                    // spaceCollide는 복잡해서 일단 스킵
                    // 대신 BlockCollisionManager의 직접 충돌 검사 사용
                } catch (Exception e) {
                    // ignore
                }
            }
